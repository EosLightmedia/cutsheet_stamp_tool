...
def lay_branding(self, prepared_by: int):
    ...
    for page_number, page in enumerate(pdfreader_obj.pages, start=1):
        ...
        # Check if page.Parent and page.Parent.MediaBox are not None
        if page.Parent is not None and page.Parent.MediaBox is not None:
            page_width, page_height = int(page.Parent.MediaBox[2]), int(page.Parent.MediaBox[3])
        else:
            raise ValueError("Page Parent or MediaBox is None, cannot determine page dimensions.")
